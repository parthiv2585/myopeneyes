...
class Candidates_model extends CI_Model {
    ...
    /**
     * Get list of non-deleted candidate
     *
     * @param  int $limit
     * @param  int $offset
     * @param  array $filters
     ...
     */
    function get_all($limit = 0, $offset = 0, $filters = array(), $sort = 'candidate_id', $dir = 'ASC',$id = NULL)
    { 
		...
		if(isset($id) && !empty($id)) {
			$sql .= " RIGHT	 JOIN candidate_assign as CA ON C.candidate_id = CA.candidate_id "; 
		}
        ...
        {             
            $sql = "
                INSERT INTO {$this->_db} (
					...
					" . $this->db->escape($data['middle_name']) . ",
					" . $this->db->escape($data['phone_home']) . ",
					" . $this->db->escape($data['phone_cell']) . ",
					" . $this->db->escape($data['phone_work']) . ",
					...
					" . $this->db->escape(isset($data['can_relocate']) ? $data['can_relocate'] : 0 ) . ",
					" . $this->db->escape($data['notes']) . ",
                    " . $this->db->escape($data['key_skills']) . ",
					" . $this->db->escape($data['current_employer']) . ",                    
                    '" . date('Y-m-d H:i:s') . "',
                    '" . date('Y-m-d H:i:s') . "',
					'1',
					'1',
					" . $this->db->escape($data['email1']) . ",
					" . $this->db->escape($data['email2']) . ",
					" . $this->db->escape($data['web_site']) . ",
					" . $this->db->escape($data['desired_pay']) . ",
					" . $this->db->escape($data['current_pay']) . ",
					" . $this->db->escape($data['best_time_to_call']) . ",
					
					" . $this->db->escape($data['work_authorization']) . ",
					" . $this->db->escape($data['date_of_birth']) . ",
					" . $this->db->escape($data['ssn']) . "
                )
            "; 
			...
			$sql_att = "
                INSERT INTO attachment (
					...
					" . $id . ",
					'100',  
					".$this->db->escape($resumeNameGetAfterUpload).",
					".$this->db->escape($resumeNameGetAfterUpload).",
					'1',
					" . $this->db->escape($data['resume-test']) . " 
                )
            ";   
