...
class Candidateassigns_model extends CI_Model {
     ...
     */
    function add_candidateassign($popup_joborder_id,$popup_client_id,$candidate_id)
    {  
	    $this->db->trans_begin();	
		
		if ($candidate_id)
		{    
			$sqlT = "SELECT * FROM candidate_assign WHERE 
					candidate_id =  '".$candidate_id."' AND
					joborder_id =  '".$popup_joborder_id."' AND 
			...
			if ($queryT->num_rows()){  
				...
				'joborder_id' => $popup_joborder_id,				
				'client_id' => $popup_client_id, 
				'created_by' => $this->session->userdata['logged_in']['user_id'] 				 
				);   
				$this->db->insert('candidate_assign',$data);			
